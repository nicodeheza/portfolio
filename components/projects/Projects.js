	...
	return (
		<section id={id} className={styles.sectionContainer}>
			<div className={styles.eleContainer}>
				<div className={styles.titleContainer} onClick={() => setShowArticle(true)}>
					<h1>{projectData.title}</h1>
					<h2>{projectData.subtitle}</h2>
				</div>
				<div className={styles.imageContainer} onClick={() => setShowArticle(true)}>
					<Image
			...
			{showArticle ? (
				<Article setShowArticle={setShowArticle} text={text} projectData={projectData} />
			) : null}
		</section>
	);
}
