...
export default function Article({setShowArticle, text, projectData}) {
	...
	return (
		<div className={styles.mainContainer}>
			<article
				className={`${styles.articleContainer} ${eleMount ? styles.open : styles.close}`}
				...
				<div className={styles.contentContainer}>
					...
					<div className={styles.links}>
						<a href={projectData.gitHubUrl} target="_blank" rel="noreferrer">
							{text[1]} <GitHubLogo classN={styles.gitHubLogo} />
						</a>
						<a href={projectData.demoUrl} target="_blank" rel="noreferrer">
							{text[2]} <WebLogo classN={styles.webLogo} />
						</a>
					</div>
				</div>
			</article>
