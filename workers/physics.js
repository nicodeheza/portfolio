...
Body.boxes.forEach((box) => {
	const boxBody = new CANNON.Body({
		...
		shape: new CANNON.Box(
			new CANNON.Vec3(box.dimensions.x, box.dimensions.y, box.dimensions.z)
		)
	});
...
const shape = [
	...
	{type: "cylinder", geometry: new CANNON.Cylinder(0.1, 0.1, 0.5, 25)}
];
for (let i = 0; i < 30; i++) {
...
function createParticle(shape) {
	...
	body.applyLocalImpulse(
		new CANNON.Vec3(randomNum(-0.1, 0.2), randomNum(-0.1, 0.2), randomNum(-0.1, 0.2)),
